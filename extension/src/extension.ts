...
async function updateVisibilityContexts() {
  ...
  await Promise.all(
    (Object.keys(VISIBILITY_KEYS) as Array<keyof typeof VISIBILITY_KEYS>).map(
      (key) =>
...
function getCustomLaunchers(): CustomLauncher[] {
  const config = vscode.workspace.getConfiguration("openTerminalEditor");
  const entries = config.get<CustomLauncher[]>("customLaunchers", []);
  return Array.isArray(entries) ? entries : [];
}

async function saveCustomLaunchers(entries: CustomLauncher[]) {
  const config = vscode.workspace.getConfiguration("openTerminalEditor");
  await config.update("customLaunchers", entries, vscode.ConfigurationTarget.Global);
}

function resolveCustomIcon(icon: string): { type: "codicon" | "url"; value: string } | null {
...
class LlmPanelProvider implements vscode.WebviewViewProvider {
  ...
  resolveWebviewView(view: vscode.WebviewView) {
    ...
    view.webview.onDidReceiveMessage(async (message) => {
      if (message?.type === "toggle") {
        const config = vscode.workspace.getConfiguration("openTerminalEditor");
        const key = message.key as keyof typeof VISIBILITY_KEYS | undefined;
        if (key && Object.prototype.hasOwnProperty.call(VISIBILITY_KEYS, key)) {
          await config.update(
            VISIBILITY_KEYS[key].replace("openTerminalEditor.", ""),
  ...
  private getHtml(webview: vscode.Webview): string {
    ...
    const state = getVisibilityState();
    type Note = { text?: string; url?: string; label?: string };
    type Entry = {
      ...
      .map((entry) => {
        ...
        <section class="card custom">
          <div class="header">
            ...
            <button class="link-button" data-custom-run="true" data-name="${escapeAttribute(
              entry.name
            )}" data-command="${escapeAttribute(entry.command)}" data-icon="${escapeAttribute(
              entry.icon
            )}" type="button">Run</button>
            <button class="danger-button" data-custom-delete="true" data-id="${escapeAttribute(
              entry.id
...
export function activate(context: vscode.ExtensionContext) {
  const codexIcon = vscode.Uri.joinPath(
    context.extensionUri,
    "resources",
    "codex.svg"
  );
  const geminiIcon = vscode.Uri.joinPath(
    context.extensionUri,
    "resources",
    "gemini.svg"
  );
  const openCodeIcon = vscode.Uri.joinPath(
    context.extensionUri,
    "resources",
    "opencode.svg"
  );
  const openSpecIcon = vscode.Uri.joinPath(
    context.extensionUri,
    "resources",
    "openspec.svg"
  );
  const qwenIcon = vscode.Uri.joinPath(
    context.extensionUri,
    "resources",
    "qwen.svg"
  );
  const claudeIcon = vscode.Uri.joinPath(
    context.extensionUri,
    "resources",
    "claude.svg"
  );
